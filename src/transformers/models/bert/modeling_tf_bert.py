...
class TFBertEncoder(tf.keras.layers.Layer):
    ...
    def _get_aux_hidden_and_attn_matrices(self, aux_tok_positions, hidden_states, aux_attn_mask, attention_mask):
        ...
        if self.config.is_diagnostics:
            if self.config.num_aux_toks == 0:
                assert hidden_state_gating_block.shape[1] == self.config.max_seq_len, f"{hidden_state_gating_block.shape[1]} {self.config.max_seq_len}"
                print(f"hidden_state_gating_block == hidden_states")
            else:
                assert hidden_state_gating_block.shape[1] == self.config.num_aux_toks + self.config.max_seq_len, f"{hidden_state_gating_block.shape[1]} " \
        ...
        if self.config.is_diagnostics:
            if self.config.num_aux_toks == 0:
                ...
                assert attention_mask_gating_block.shape[3] == self.config.num_aux_toks+self.config.max_seq_len, f"{attention_mask_gating_block.shape[3]} " \
                                                                                                                 f"{self.config.num_aux_toks+self.config.max_seq_len}"
